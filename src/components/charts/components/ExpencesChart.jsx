...
// Row 59 = Total Expenses (not row 52 which is Total Below GP Expenses)
const KPI_ROWS = [59, 54, 56];
...
const ExpencesChart = ({ tableData, selectedPeriods, computeCellValue, style, hideHeader = false }) => {
  ...
  const variances = cards.map((card, idx) => {
    ...
    const currentPeriod = periodsToUse[idx];
    const prevPeriod = periodsToUse[idx - 1];
    return calcVariance(card.value, cards[idx - 1].value, currentPeriod?.type, prevPeriod?.type);
  });

  return (
    <div className="expenses-trend-container" style={style || {}}>
      {!hideHeader && (
        <h2 className="trend-heading">Cost & Profitability Trend</h2>
      )}
      
      {/* Total Expenses row title */}
      <h3 className="trend-kpi-title">Total Expenses</h3>
