...
const EntityEmailThread = ({ customerId, prospectId, inquiryId, title = 'Email Thread' }) => {
  ...
  const requestParams = useMemo(() => {
    ...
    if (prospectId) params.prospect_id = prospectId;
    if (inquiryId) params.inquiry_id = inquiryId;
    return params;
  }, [customerId, prospectId, inquiryId]);
  ...
  const loadEmails = useCallback(async () => {
    ...
    try {
      ...
      console.error('Failed to load entity emails', error);
  ...
  return (
    <Space direction="vertical" size={10} style={{ width: '100%' }}>
      ...
      {!emails.length && !loading ? (
        <Alert type="info" showIcon message="No linked emails yet" description="Emails sent from CRM will appear here for this record." />
      ) : null}
...
export default EntityEmailThread;
