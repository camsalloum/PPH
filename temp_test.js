...
function detectCombinedComponentLayout(rawText) {
  ...
  const pairMatch = text.match(/\b([A-Z]{1,5}\d{2,6}[A-Z]?)\s*\+\s*([A-Z]{1,5}\d{2,6}[A-Z]?)\b/);
  let codeA = pairMatch ? String(pairMatch[1]).toUpperCase() : null;
  let codeB = pairMatch ? String(pairMatch[2]).toUpperCase() : null;

  if (!codeA) {
    const partA = text.match(/\b(?:part|component)\s*a\b[^\n]{0,40}\b([A-Z]{1,5}\d{2,6}[A-Z]?)\b/i);
    if (partA) codeA = String(partA[1]).toUpperCase();
  }

  if (!codeB) {
    const partB = text.match(/\b(?:part|component)\s*b\b[^\n]{0,40}\b([A-Z]{1,5}\d{2,6}[A-Z]?)\b/i)
      || text.match(/\bhard(?:e)?n(?:er)?\b[^\n]{0,40}\b([A-Z]{1,5}\d{2,6}[A-Z]?)\b/i);
    if (partB) codeB = String(partB[1]).toUpperCase();
  }
...
function selectTwoNumericCandidates(values, def, sourceText = '') {
  ...
  } else if (unit.includes('cps') || unit.includes('cp')) {
  ...
  // Two-column extraction is only reliable when exactly two values exist.
  ...
  if (filtered.length === 2) {
    ...
    if (hasToleranceToken) {
      ...
      if (unit.includes('%') && percentCount >= 3 && !hasTinyValue) {
...
module.exports = function (router) {
  ...
  router.put('/tds/non-resin-spec/bulk-apply', authenticate, async (req, res) => {
    ...
    try {
      ...
      for (let idx = 0; idx < entries.length; idx += 1) {
        ...
        const validation = validateNonResinParameters(materialClass, mergedParams, context, rulesOverride);
        if (!validation.valid) {
  ...
  router.post('/tds/non-resin-spec/parse-upload', authenticate, upload.single('file'), async (req, res) => {
    ...
    try {
      ...
      const multiComponentMode = mode === 'multi_component' && materialClass === 'adhesives';
      ...
      if (multiComponentMode) {
        ...
        const hasMeaningfulComponents = componentSeeds.filter(
          (c) => Object.keys(c.extracted || {}).length > 0
        ).length >= 2;
