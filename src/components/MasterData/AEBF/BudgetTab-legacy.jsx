...
const BudgetTab = () => {
  ...
  // Format tabs (Excel vs HTML vs Sales Rep Recap)
  const [activeFormatTab, setActiveFormatTab] = useState('excel');
  
  // HTML Format sub-tabs (Divisional vs Sales Reps)
  ...
  const [isAllSalesReps, setIsAllSalesReps] = useState(false);
  ...
  const [htmlProductGroups, setHtmlProductGroups] = useState([]); // List of product groups for division
  ...
  // Fetch countries from country reference master list
  const fetchHtmlCountries = useCallback(() => {
    try {
      const allCountries = Object.keys(countryCoordinates || {});
      const sortedCountries = allCountries.sort((a, b) => a.localeCompare(b));
      setHtmlCountries(sortedCountries);
    } catch (error) {
      console.error('Error loading country reference list:', error);
      setHtmlCountries([]);
    }
  }, []);
  ...
  const filterDropdownOptions = useMemo(() => {
    ...
    const salesRepLabels = [...new Set(htmlTableData.map(row => getSalesRepDisplayLabel(row.salesRep)).filter(Boolean))].sort();
    const customers = [...new Set(htmlTableData.map(row => row.customer).filter(Boolean))].sort();
    const countries = [...new Set(htmlTableData.map(row => row.country).filter(Boolean))].sort();
    const productGroups = [...new Set(htmlTableData.map(row => row.productGroup).filter(Boolean))].sort();
    
    return { salesReps: salesRepLabels, customers, countries, productGroups };
  }, [htmlTableData, isAllSalesReps, getSalesRepDisplayLabel]);
  ...
  // Calculate monthly Actual Amount totals for Sales Rep (MT * 1000 * sellingPrice)
  const htmlMonthlyActualAmountTotals = useMemo(() => {
    ...
    return totals;
  }, [htmlTableData, filteredHtmlTableData, isAllSalesReps, findHtmlPricing]);
  ...
  const fetchHtmlSalesRepOptions = async (
    ...
    try {
      ...
      const response = await axios.post('/api/sales-rep-divisional-ultra-fast', {
  ...
  const fetchHtmlFilterOptions = async () => {
    if (!selectedDivision || activeFormatTab !== 'html') return;
    
    setHtmlFiltersLoading(true);
    try {
      const [filterResponse, groupsResponse, salesRepsResponse] = await Promise.all([
        axios.get('/api/aebf/filter-options', {
          params: { division: selectedDivision, type: 'Actual' }
        }),
        axios.get('/api/sales-rep-groups-universal', {
          params: { division: selectedDivision }
        }),
        axios.get('/api/sales-reps-universal', {
          params: { division: selectedDivision }
        })
      ]);
  ...
  const fetchData = async (page = 1, pageSize = 50, searchFilter = null) => {
    ...
    try {
      ...
      const response = await axios.get('/api/aebf/budget', { params });
  ...
  const handleExport = async () => {
    ...
    try {
      const params = {
        division: selectedDivision,
        types: 'Budget',  // Specify Budget type for export
      };
      ...
      window.open(`/api/aebf/export?${queryString}`, '_blank');
  ...
  // Optimized columns with content-based widths
  const columns = [
    {
      title: 'Year',
      dataIndex: 'year',
  ...
  useEffect(() => {
    if (activeFormatTab === 'html') {
      fetchHtmlFilterOptions();
      fetchHtmlCountries();
      fetchHtmlProductGroups();
    } else if (activeFormatTab === 'recap') {
      fetchRecapYears();
    }
  }, [selectedDivision, activeFormatTab, fetchHtmlCountries]);
  
  // Auto-save draft every 30 seconds
  useEffect(() => {
    if (activeFormatTab !== 'html' || !htmlFilters.salesRep || !htmlFilters.actualYear) {
      return;
    }
  ...
  }, [activeFormatTab, htmlFilters.salesRep, htmlFilters.actualYear, htmlCustomRows, htmlBudgetData]);
  
  // Auto-save draft 5 seconds after last change
  useEffect(() => {
    if (activeFormatTab !== 'html' || !htmlFilters.salesRep || !htmlFilters.actualYear) {
      return;
    }
  ...
  // Fetch product groups for division (excluding Services Charges)
  const fetchHtmlProductGroups = async () => {
    if (!selectedDivision || !htmlFilters.salesRep) {
      setHtmlProductGroups([]);
      return;
    }
    
    try {
      const response = await axios.get('/api/product-groups-universal', {
        params: { division: selectedDivision, salesRep: htmlFilters.salesRep }
      });
  ...
  // Update product groups when sales rep changes
  useEffect(() => {
    if (htmlFilters.salesRep && selectedDivision) {
      fetchHtmlProductGroups();
    } else {
      setHtmlProductGroups([]);
    }
  }, [htmlFilters.salesRep, selectedDivision]);
  ...
  const fetchAllSalesRepsData = useCallback(async () => {
    ...
    try {
      ...
      const salesRepsResponse = await axios.get('/api/sales-reps-universal', {
        params: { division: selectedDivision }
      });
      ...
      const response = await axios.post('/api/aebf/html-budget-customers-all', {
        division: selectedDivision,
  ...
  const fetchHtmlTableData = useCallback(async () => {
    ...
    setHtmlTableLoading(true);
    try {
      const response = await axios.post('/api/aebf/html-budget-customers', {
        division: selectedDivision,
        actualYear: htmlFilters.actualYear,
      ...
      if (response.data.success) {
        ...
        (response.data.data || []).forEach(row => {
          for (let month = 1; month <= 12; month++) {
            const key = `${row.customer}|${row.country}|${row.productGroup}|${month}`;
            // Use budget data from backend if available, otherwise empty string
  ...
  useEffect(() => {
    if (activeFormatTab === 'html' && htmlFilters.actualYear && htmlFilters.salesRep) {
      if (isAllSalesReps) {
        fetchAllSalesRepsData();
      } else {
        fetchHtmlTableData();
      }
    } else if (activeFormatTab === 'html') {
      setHtmlTableData([]);
      setHtmlBudgetData({});
      setAllSalesRepsTableData([]);
    }
    // Clear custom rows when switching away from HTML tab or when filters change
    if (activeFormatTab !== 'html') {
      setHtmlCustomRows([]);
      setNewCustomerInputs({});
    }
  }, [htmlFilters.actualYear, htmlFilters.salesRep, activeFormatTab, isAllSalesReps, fetchHtmlTableData, fetchAllSalesRepsData]);
  ...
  const handleSaveBudget = async () => {
    ...
    setHtmlSaving(true);
    try {
      const response = await axios.post('/api/aebf/save-html-budget', {
        division: selectedDivision,
    ...
    try {
      ...
      if (isAllSalesReps) {
        ...
        for (const salesRep of salesRepsList) {
          ...
          try {
            // First save as draft
            await axios.post('/api/budget-draft/save-draft', {
              division: selectedDivision,
              salesRep: salesRep,
              budgetYear: parseInt(htmlFilters.actualYear) + 1,
            ...
            if (response.data.success) {
              ...
              try {
                await axios.delete(
                  `/api/budget-draft/delete-draft/${selectedDivision}/${salesRep}/${parseInt(htmlFilters.actualYear) + 1}`
                );
              } catch (e) {
      ...
      if (response.data.success) {
        ...
        try {
          await axios.delete(
            `/api/budget-draft/delete-draft/${selectedDivision}/${htmlFilters.salesRep}/${parseInt(htmlFilters.actualYear) + 1}`
          );
          setHasDraft(false);
          setDraftStatus('saved');
        } catch (deleteError) {
  ...
  const handleExportHtmlForm = async () => {
    // In All Sales Reps mode, require targetSalesRep for single export
    if (isAllSalesReps && !targetSalesRep) {
      message.warning('Please select a Target Sales Rep to export, or use "Export All" for combined export');
      return;
    }
    
    const effectiveSalesRep = isAllSalesReps ? targetSalesRep : htmlFilters.salesRep;
    
    if (!selectedDivision || !htmlFilters.actualYear || !effectiveSalesRep) {
      message.warning('Please select all filters first');
      return;
    }
    
    // In All mode, filter data for the target sales rep
    const filteredTableData = isAllSalesReps 
      ? htmlTableData.filter(row => row.salesRep === targetSalesRep)
      : htmlTableData;
    
    // Filter budget data for target sales rep (in All mode, keys have salesRep| prefix)
    ...
    const customRowsData = htmlCustomRows
      ...
      .filter(row => !isAllSalesReps || row.salesRep === targetSalesRep) // Filter by salesRep in All mode
      .map(row => ({
    ...
    try {
      const response = await axios.post(
        '/api/aebf/export-html-budget-form',
        {
          division: selectedDivision,
          actualYear: htmlFilters.actualYear,
      ...
      message.success('Budget form exported successfully');
    } catch (error) {
  ...
  const fetchDivisionalHtmlActualYears = useCallback(async () => {
    ...
    try {
      const response = await axios.get('/api/aebf/html-budget-actual-years', {
        params: { division: selectedDivision }
      });
  ...
  const fetchDivisionalHtmlTableData = useCallback(async (options = {}) => {
    ...
    try {
      const response = await axios.post('/api/aebf/divisional-html-budget-data', {
        division: selectedDivision,
      ...
      if (response.data.success) {
        ...
        if (Object.keys(budgetDataFromBackend).length > 0 && !skipSuccessMessage) {
          message.success(`Loaded existing divisional budget data (${Object.keys(budgetDataFromBackend).length} entries)`);
        }
      } else {
  ...
  const handleExportDivisionalHtmlForm = async () => {
    ...
    try {
      const response = await axios.post(
        '/api/aebf/export-divisional-html-budget-form',
        {
          division: selectedDivision,
          actualYear: divisionalHtmlFilters.actualYear,
  ...
  const handleImportDivisionalFilledHtml = async (file) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        const checkResponse = await axios.post('/api/aebf/import-divisional-budget-html', {
  ...
  const performDivisionalImport = async (htmlContent) => {
    try {
      const response = await axios.post('/api/aebf/import-divisional-budget-html', {
        htmlContent: htmlContent,
        confirmReplace: true
      });
  ...
  const submitDivisionalBudget = async () => {
    ...
    const budgetYear = divisionalBudgetYearDerived;
    if (!budgetYear) {
      message.warning('Unable to determine budget year');
      return;
    }
    ...
    try {
      const response = await axios.post('/api/aebf/save-divisional-budget', {
        division: selectedDivision,
        budgetYear,
        records
      });
      ...
      // Calculate total MT from KGS records (divide by 1000 to convert back to MT)
      const totalKgsRecords = response.data.recordsInserted?.kgs || 0;
      ...
      const divisionName = response.data.metadata?.division || selectedDivision;
      ...
      notification.success({
        message: 'Divisional Budget Saved',
        description: `${formattedMT} MT added to ${divisionName} ${budgetYear} database (${totalKgsRecords} product-month records)`,
        duration: 6,
        placement: 'topRight'
      });
  ...
  const divisionalBudgetYearDerived = divisionalHtmlFilters.actualYear ? Number(divisionalHtmlFilters.actualYear) + 1 : null;
  
  // Load divisional actual years and product groups on mount/division change
  useEffect(() => {
    if (selectedDivision && activeFormatTab === 'html' && activeHtmlSubTab === 'divisional') {
      fetchDivisionalHtmlActualYears();
    }
  }, [selectedDivision, activeFormatTab, activeHtmlSubTab, fetchDivisionalHtmlActualYears]);
  
  // Fetch divisional table data when filters change
  useEffect(() => {
    if (activeFormatTab === 'html' && activeHtmlSubTab === 'divisional' && divisionalHtmlFilters.actualYear) {
      fetchDivisionalHtmlTableData();
    } else if (activeFormatTab === 'html' && activeHtmlSubTab === 'divisional') {
      setDivisionalHtmlTableData([]);
      setDivisionalHtmlBudgetData({});
    }
  }, [divisionalHtmlFilters.actualYear, activeFormatTab, activeHtmlSubTab, fetchDivisionalHtmlTableData]);
  ...
  const fetchRecapYears = async () => {
    ...
    try {
      const response = await axios.get('/api/aebf/budget-years', {
        params: { division: selectedDivision }
      });
  ...
  const fetchRecapSalesReps = async (year) => {
    ...
    try {
      const response = await axios.get('/api/aebf/budget-sales-reps', {
      ...
      if (response.data.success) {
        ...
        Object.keys(htmlGroups).forEach(groupName => {
          ...
          const hasData = rawSalesReps.some(rep => normalizedMembers.includes(rep.toString().trim().toUpperCase()));
          if (hasData) {
        ...
        // Add individual sales reps (not in groups)
        rawSalesReps.forEach(rep => {
          const normalized = rep.toString().trim().toUpperCase();
          if (!groupedMembers.has(normalized)) {
  ...
  const fetchRecapData = async (year, salesRep) => {
    ...
    if (!salesRep || salesRep === '__ALL__') {
      ...
      try {
        ...
        const salesRepsResponse = await axios.get('/api/aebf/budget-sales-reps', {
          params: { division: selectedDivision, budgetYear: year }
        });
        ...
        const recapPromises = allSalesReps.map(rep => 
          axios.post('/api/aebf/budget-sales-rep-recap', {
            division: selectedDivision,
            budgetYear: year,
            salesRep: rep
          })
    ...
    try {
      ...
      if (groupMembers && groupMembers.length > 0) {
        // Fetch recap for each member and aggregate
        const recapPromises = groupMembers.map(member => 
          axios.post('/api/aebf/budget-sales-rep-recap', {
            division: selectedDivision,
            budgetYear: year,
            salesRep: member
        ...
        // Individual sales rep - find raw name from label map
        let rawSalesRep = salesRep;
        
        // Reverse lookup in label map
        ...
        const response = await axios.post('/api/aebf/budget-sales-rep-recap', {
          division: selectedDivision,
          budgetYear: year,
          salesRep: rawSalesRep
        });
  ...
  const fetchRecapProductGroups = async (year, salesRep) => {
    ...
    try {
      ...
      const pgPromises = rawSalesReps.map(rep =>
        axios.post('/api/aebf/budget-product-groups', {
          division: selectedDivision,
          budgetYear: year,
          salesRep: rep
        })
      ...
      if (!salesRep || salesRep === '__ALL__') {
        ...
        const actualResponse = await axios.post('/api/aebf/actual-product-groups', {
          division: selectedDivision,
        ...
        const actualPromises = rawSalesReps.map(rep =>
          axios.post('/api/aebf/actual-product-groups', {
            division: selectedDivision,
            actualYear: actualYear,
            salesRep: rep
          })
      ...
      setRecapProductGroups(Object.values(pgMap));
    } catch (error) {
      console.error('Error fetching product groups:', error);
  ...
  const handleDeleteBudget = () => {
    ...
    // In All Sales Reps mode, require targetSalesRep
    if (isAllSalesReps && !targetSalesRep) {
      message.warning('Please select a Target Sales Rep first to delete their budget');
      return;
    }
    
    const effectiveSalesRep = isAllSalesReps ? targetSalesRep : htmlFilters.salesRep;
    
    if (!selectedDivision || !effectiveSalesRep || !htmlFilters.actualYear) {
      message.warning('Please select all filters first');
      return;
    }
    ...
    (async () => {
      try {
        const url = `/api/budget-draft/delete-final/${selectedDivision}/${effectiveSalesRep}/${budgetYear}`;
  ...
  const handleDeleteDivisionalBudget = () => {
    ...
    (async () => {
      try {
        const url = `/api/aebf/delete-divisional-budget/${encodeURIComponent(selectedDivision)}/${budgetYear}`;
  ...
  // Get customer country mapping from table data
  const getCustomerCountryMap = useMemo(() => {
    const map = {};
    htmlTableData.forEach(row => {
      if (!map[row.customer]) {
        map[row.customer] = row.country;
      }
    });
    return map;
  }, [htmlTableData]);
    ...
    <>
      ...
      >
        {uploadStep === 1 ? (
          ...
          <Spin spinning={uploading} tip="Uploading and processing budget data...">
        ...
        <Spin spinning={divisionalHtmlFiltersLoading}>
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: '16px', flexWrap: 'wrap' }}>
            ...
            <div style={{ flex: '1', minWidth: '120px', maxWidth: '120px' }}>
              ...
              <Input
                value={divisionalBudgetYearDerived ? divisionalBudgetYearDerived.toString() : ''}
                placeholder="—"
                disabled
                size="middle"
          ...
          {divisionalHtmlFilters.actualYear && Object.keys(divisionalHtmlBudgetData).length > 0 && (
            <div style={{ 
              marginTop: '10px', 
              padding: '6px 12px', 
              background: divisionalDraftStatus === 'saved' ? '#f6ffed' : divisionalDraftStatus === 'saving' ? '#fff7e6' : '#fff2f0',
              borderRadius: '4px',
              display: 'flex',
              alignItems: 'center',
              gap: '8px',
              ...
              <span>
                {divisionalDraftStatus === 'saving' && '💾 Saving...'}
                {divisionalDraftStatus === 'saved' && '✅ Draft saved'}
                {divisionalDraftStatus === 'error' && '⚠️ Save failed'}
              </span>
              {divisionalLastSaveTime && divisionalDraftStatus === 'saved' && (
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '24px' }}>
                ...
                <div style={{ marginLeft: 'auto' }}>
                  <span style={{ 
                    fontSize: '14px', 
                    fontWeight: 700, 
                    color: (divisionalBudgetYearTotal - divisionalActualYearTotal) >= 0 ? '#52c41a' : '#ff4d4f' 
                  }}>
                    {((divisionalBudgetYearTotal - divisionalActualYearTotal) / (divisionalActualYearTotal || 1) * 100).toFixed(0)}%
                  </span>
                </div>
              </div>
            </div>
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '24px' }}>
                ...
                <div style={{ marginLeft: 'auto' }}>
                  <span style={{ 
                    fontSize: '14px', 
                    fontWeight: 700, 
                    color: (divisionalAmountYearTotal - divisionalActualAmountYearTotal) >= 0 ? '#52c41a' : '#ff4d4f' 
                  }}>
                    {((divisionalAmountYearTotal - divisionalActualAmountYearTotal) / (divisionalActualAmountYearTotal || 1) * 100).toFixed(0)}%
                  </span>
                </div>
              </div>
            </div>
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '24px' }}>
                ...
                <div style={{ marginLeft: 'auto' }}>
                  <span style={{ 
                    fontSize: '14px', 
                    fontWeight: 700, 
                    color: (divisionalMormYearTotal - divisionalActualMormYearTotal) >= 0 ? '#52c41a' : '#ff4d4f' 
                  }}>
                    {((divisionalMormYearTotal - divisionalActualMormYearTotal) / (divisionalActualMormYearTotal || 1) * 100).toFixed(0)}%
                  </span>
                </div>
              </div>
            </div>
          ...
          <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '24px' }}>
                ...
                <div style={{ marginLeft: 'auto' }}>
                  <span style={{ 
                    fontSize: '14px', 
                    fontWeight: 700, 
                    color: (htmlBudgetYearTotal - htmlActualYearTotal) >= 0 ? '#52c41a' : '#ff4d4f' 
                  }}>
                    {((htmlBudgetYearTotal - htmlActualYearTotal) / (htmlActualYearTotal || 1) * 100).toFixed(0)}%
                  </span>
                </div>
              </div>
            </div>
            ...
            <div style={{ 
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '24px' }}>
                ...
                <div>
                  <span style={{ fontSize: '11px', color: '#999' }}>Bud: </span>
                  <span style={{ fontSize: '16px', fontWeight: 600, color: '#d4b106' }}>{formatAed(htmlAmountYearTotal)}</span>
                </div>
                <div style={{ marginLeft: 'auto' }}>
                  <span style={{ 
                    fontSize: '14px', 
                    fontWeight: 700, 
                    color: (htmlAmountYearTotal - htmlActualAmountYearTotal) >= 0 ? '#52c41a' : '#ff4d4f' 
                  }}>
                    {((htmlAmountYearTotal - htmlActualAmountYearTotal) / (htmlActualAmountYearTotal || 1) * 100).toFixed(0)}%
                  </span>
                </div>
              </div>
            </div>
                         ...
                         <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                           ...
                           <>
                             <Button
                               ...
                               title={isAllSalesReps && !targetSalesRep ? 'Select a Target Rep first' : 'Delete budget data'}
                             >
                               Delete
                             </Button>
                             <Button
  ...
  return (
    <div className="budget-tab" style={{ padding: '0', width: '100%' }}>
      {/* Format Tabs - Compact */}
      <Tabs
        activeKey={activeFormatTab}
        onChange={setActiveFormatTab}
        size="small"
